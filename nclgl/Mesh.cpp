...
Mesh* Mesh::GenerateCircle(){
	...
	m->vertices[0] = 	Vector3(0.f, 0.f, 0.0f);
	m->textureCoords[0] = Vector2(0.5f,	0.5f);

	for(int i = 0; i < m->numVertices; i ++){
		m->vertices[i+1] = Vector3 (cosf(i * Pi / 18.f), sinf(i * Pi / 18.f), 0);
		m->colours[i+1]= Vector4(1,1,1,1);
		m->textureCoords[i+1] = Vector2((cosf(i * Pi / 18.f)+1)/2.f,(sinf(i * Pi / 18.f)+1)/2.f);
	}
