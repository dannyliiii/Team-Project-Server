...
bool CollisionHelper::SphereCapsuleCollision(PhysicsNode& p1, PhysicsNode& p0, CollisionData* data) {
	
	Matrix4 rotation = p0.GetOrientation().Conjugate().ToMatrix();
	...
	Vector3 endPoint1 = (p1.GetPosition() - rotation * Vector3(0, 0, 0));
	Vector3 endPoint2 = (p1.GetPosition() + rotation * Vector3(0, capsule.GetHeight(), 0));

	Vector3 cylinderCentre = rotation * Vector3(0, capsule.GetHeight(), 0);
	float distanceFromEp1 = (p0.GetPosition() - endPoint1).Length() / cylinderCentre.Length();
	if (distanceFromEp1 < 0) distanceFromEp1 = 0;
	if (distanceFromEp1 > 1) distanceFromEp1 = 1;
	Vector3 closestPoint = endPoint1 + (cylinderCentre * distanceFromEp1);

	Vector3 collisionVector = p0.GetPosition() - closestPoint;
	float distance = collisionVector.Length();
	Vector3 collisionNormal = collisionVector / distance;

	const float sumRadius = (sphere.GetRadius() + capsule.GetRadius());
	...
	if (distance < sumRadius)
	{
		if (data)
		{
			...
			data->m_normal = -Vector3::Normalise(p0.GetPosition() - closestPoint);
			data->m_point = p0.GetPosition() - data->m_normal * (sphere.GetRadius() - data->m_penetration*0.5f);
		}
		return true;
